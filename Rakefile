...
namespace :db do
  task :seed do
    # Hamlet
    hamlet = Play.find_or_create_by_title(:title => "Hamlet")
    hamlet.update_attributes(
    ...
    addDates(hamlet.id,1340597333)

    # Romeo & Juliet
    raj = Play.find_or_create_by_title(:title => "Romeo and Juliet")
    raj.update_attributes(
    ...
    addDates(raj.id,1340597333+9000,10)
...
def addDates(play_id, ts, amount=20)
  amount.times do |i|
    playtime = ts+(i*(24*60*60))
    PlayDate.find_or_create_by_playtime_and_play_id(playtime, play_id).update_attributes(:price => 50.0)
  end
end